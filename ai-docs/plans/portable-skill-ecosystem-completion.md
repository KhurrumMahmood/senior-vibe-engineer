...
| 2026-07-16 | Codex | Native non-model discovery probes closed two WP3 runtime uncertainties without invoking an external model: isolated Codex 0.144.1 plugin install followed by `debug prompt-input` exposed all 16 projected skills in the model-visible inventory; isolated Claude Code 2.1.211 plugin validation/install followed by `plugin details` reported the exact 16-skill component inventory and approximately 2,037 always-on tokens. This measurement strengthens ADR 0042's router-only default: future lifecycle evidence must show exactly two ambient bootstrap skills by default and all 16 only after explicit `full-discovery`. | No status-count change; Cursor remains the unresolved IM-11 runtime. |
