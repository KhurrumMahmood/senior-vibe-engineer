...
| WP3 Load-bearing layers, bindings, and installer | in_progress | Codex | WP1, WP2 | AC-3.1–AC-3.7 | `ai-docs/specs/portable-skill-layer-distribution.md`; `reports/portable-skill-ecosystem-completion/WP3/characterization.md` | AR-1–AR-12 are characterized at `eba43fa`: exact catalog/placement/foundation/move/discovery/cold-host/first-value oracles and gaps are frozen. Current action: implement Slice 1 IM-1/IM-2; Cursor/Augment runtime discovery remains a named completion risk, not a structural-check waiver. |
