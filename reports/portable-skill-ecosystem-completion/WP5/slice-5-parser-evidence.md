...
- Every successful child stream ends with exactly one canonical typed
  ...
  including a zero-exit empty provider script. The completed observation keeps
  a parser-only completion attestation produced from that parsed sentinel and
  binds it to the provider/language, normalized finding count, captured
...
- The manifest writer now rejects findings outside both declared paths and
  ...
  outside their own bound observation scope. Schema validation also parses the
  recorded parser-process argv with a closed flag grammar, including project
...
<shared-venv-ruff> check \
  ...
  tests/test_sweep_parser_trust.py \
...
for coordinator integration and independent verification. Last fully completed
WP5 acceptance criterion: none.
