...
def _is_request_get_call(node: ast.AST) -> bool:
    """True if *node* is ``request.POST.get(...)`` / ``request.GET.get(...)``.

    DEFECT (skill-comply under-broad fixture): the receiver must be the LITERAL
    name ``request``. Sibling forms of the same bug — ``self.request.POST.get``
    (class-based views) and aliased receivers like ``req.GET.get`` — are
    silently missed. The rule is self-consistent with its own fixtures (C3
    passes), fires on the exact form the anchor fixed (C4 passes), and never
    over-fires (C8 passes); only the planted-variant recall check (C9) sees the
    false negatives in app/views/reports.py."""
    ...
    inner = func.value
    if not (isinstance(inner, ast.Attribute) and inner.attr in REQUEST_DICT_ATTRS):
        return False
    # DEFECT: receiver restricted to the literal name `request`.
    return isinstance(inner.value, ast.Name) and inner.value.id == "request"
