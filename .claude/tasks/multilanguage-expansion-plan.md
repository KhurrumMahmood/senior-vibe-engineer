...
| J4 semantic/project | 5/6 integrated; rename active | `find-dormant`, `find-implicit-state`, `find-incomplete-sweep`, `find-semantic-duplication`, `map-subsystem`, `rename-concept` | Compiler attribution bounded to each claimed identity/relationship. Project graph, state, incomplete-sweep, and semantic-lead outcomes are integrated through `7cf9c6b`; only rename authority remains active. |
| J5 proposals/guards | 2/4 integrated; 2 active | `extract-enum`, `prevent-regression`, `propose-folder-reorganization`, `unify-shadows` | Enum and shadow proposals consume accepted producer artifacts and are integrated through `7c96057`; the exact-field guard and read-only folder proposal remain active. No row promotes before accepted producer evidence. |
