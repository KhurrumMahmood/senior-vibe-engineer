...
1. finish the product-aligned semantic review and accepted repairs;
2. complete the six proposal/mutation/guard skills;
3. integrate an exact 22/22 language-level disposition;
4. prove final mixed-host router-to-outcome journeys and review them; and
5. save transferable and non-transferable learning throughout.

## How to track this work

- Change a lane or gate to `complete` only after its evidence column names a
  committed revision and passing command or artifact.
- Keep failed evidence in the log; add the repair revision rather than erasing
  the failure.
- Parallel lanes may edit only their owned skill directories, cohort test, and
  cohort learning packet. The serial owner alone edits matrices, routers,
  shared harnesses, this tracker, and the expansion plan.
- A review finding becomes gating only when it materially threatens easy
  installation/on-demand execution, honest JavaScript support, a useful final
  outcome, source safety, or likely user experience. Put other worthwhile
  findings in `.claude/tasks/multilanguage-support-backlog.md`.
- Do not add a universal AST, shared executor, transactional installer, or
  cross-skill coordinator to close this phase.
...
The fresh product-aligned review of `f06b6c5` found two accepted gating defects:

- JavaScript command snippets in five skills did not independently resolve
  their skill root in both on-demand and optional installed layouts.
- `find-incomplete-sweep` labeled the detector output JavaScript, but its
  downstream scout treated only TypeScript as compiler-backed and could not
  reach the promised packet→triage outcome.
...
General contract for `.js`, `.jsx`, `.mjs`, and `.cjs`:

- use only host-local/native tooling already required by the accepted
  TypeScript implementation;
- distinguish checked JavaScript, JSDoc, inferred, syntax, and lexical facts;
- inventory before narrowing eligibility and make uncovered relevant files
  partial rather than clean;
- keep proposal skills read-only;
- prove exact diffs for mutation/guard skills; and
- execute from the on-demand library without ambient task-skill installation.
...
Use disposable, non-repository hosts and fresh non-context agents. The agent
...
This closeout is complete only when Gates A–D and the learning closeout are all
checked at committed revisions. Passing helper tests, code volume, or agent
activity alone does not count.
