...
- `dart pub deps --json` can describe a package-aware disposable fixture or a
  host that already owns resolved package state. `dart pub get --offline
  --enforce-lockfile` is a reproducible preflight only for a closure-owned tool
  package or disposable fixture; it must not run in the audited host.
...
- In a disposable package-aware probe, the resolved lock passed locked offline
  Pub get, fatal analysis, format check, one `package:test` test, and an exact
  executable smoke. This is feasibility evidence, not the D0 host-safe spine.
...
host. D0 reports exact SDK paths/versions; each closure-owned tool preflight
reports whether its required package artifacts are already available. A package
dependency must be either declared and locked by a copied closure-owned Dart
tool package, or already resolved by the host with a pre-existing package
configuration. It is never downloaded silently and product execution never
runs Pub resolution in the audited host.
...
D0 owns a dependency-free, pubspec-only plain-Dart fixture and the Dart profile,
doctor, role inventory, status vocabulary, source manifest, and artifact
lifecycle. It earns no skill disposition.

The fixture contains `pubspec.yaml` but no lock, package dependencies, or
generated package configuration. It has `lib/`, `bin/`, `test/`, `tool/`,
`example/`, a public barrel, relative imports/exports, one dependency-free
direct test script, one exact smoke value, and decoys for every source role.
The spine must finish without creating `.dart_tool/`, `pubspec.lock`, or any
other host file.

Eligible core-Dart source is authored `.dart` below declared package roots.
Inventory, but exclude from findings by default: `test/`, `integration_test/`,
...
The exact host-safe D0 native matrix is:

1. hash the pubspec, selected source, and absence/presence of host-owned package
   configuration before execution;
2. `dart analyze --fatal-infos --fatal-warnings .`;
3. `dart format --output=none --set-exit-if-changed` over existing authored
   source roots only;
4. `dart <direct-test-script.dart>` with no `package:test` dependency;
5. `dart <smoke-entrypoint.dart>` with exact stdout; and
6. re-hash the host and prove zero writes, including no new `.dart_tool/` or
   lockfile.

Any Pub get/deps/test obligation belongs to a closure-owned tool package or a
disposable package-aware skill fixture, never D0 or the audited host. A
pre-existing host package configuration may be hashed and passed to LSP; if a
needed one is absent or unresolved, package-aware semantics are partial.
...
1. **Dart project snapshot (D1-D8):** selected package roots, roles and
   exclusions, SDK/tool versions, pubspec and optional pre-existing package
...
Dart-owned: initialization capabilities, selected package roots, diagnostics,
...
|---|---|---|---|---|---|---|
| `adapt-project` | `adapter.yml`, `adapter.json`, `report.md`, `evidence.json` count authored Dart roles, package markers, and exact native commands without applying host writes. | Dependency-free pubspec-only fixture with library/bin/tool/test/example/generated roles and direct test/smoke scripts. | A conventional one-package core-Dart host yields complete objective facts and no standardization caution. | Dependency names do not imply Flutter/frameworks; tests, examples, generated, cache, build, vendor, reports, and symlinks do not count as authored library roots. | Host-safe D0 matrix; assert reported commands are executed and create no host state. | Describes observed layout only; no architecture endorsement, semantic graph, package graph, or framework inference. |
| `find-concept-divergence` | `findings.jsonl`, `report.md`, `findings.json`, `scan.json` report one glossary avoid-term with exact span/hash. | An old term in authored Dart prose/identifier plus the preferred term elsewhere and decoys in every excluded role. | Preferred term only produces complete zero findings. | Substrings, unrelated homonyms, generated/test/example/vendor/build/report/symlink content, and package names outside the declared glossary do not fire. | Host-safe D0 analyze/format/direct-test/smoke and zero-write check; findings preserve source bytes. | Strict text/glossary evidence only; no symbol identity, conceptual equivalence, or rename-completeness claim. |
| `find-folder-topology-drift` | `detections.jsonl`, `report.md`, `findings.json`, `scan.json` report one policy-backed direct-sibling filename cluster. | Three authored `billing_*.dart` siblings below one explicit `lib/src` root plus excluded decoys. | Two siblings or an explicitly cohesive/allowed folder is clean. | Barrels (`<folder>.dart`), tests, parts/generated files, bin/tool/example/vendor/build/report/symlink paths, below-threshold clusters, and nested cousins do not fire. | Host-safe D0 matrix and exact inventory/hash/zero-write assertions. | Filename topology is advisory; it proves no library ownership, import impact, move safety, or Flutter convention. |
...
| Skill | Useful final outcome and artifacts | Positive fixture obligation | Clean case | Must not fire | Native verification | Honest limitation |
|---|---|---|---|---|---|---|
| `extract-enum` | `targets.json`, `profile.md`, `proposal.md` turn one accepted state candidate into an implementation-ready Dart enum proposal without editing source. | Accepted D5 finding with exact owner/field/callers/literals plus JSON/wire and public compatibility boundaries. | Accepted evidence says an enum/sealed type already owns the state, yielding an explicit no-proposal result. | Partial/stale findings, open domains, ambiguous receiver/field, generated owners, external API ownership, reflection/serialization uncertainty, and unaccepted candidates do not produce a proposal. | Revalidate hashes; apply the exact proposal only in a disposable copy and run host-safe D0 matrix there. | Proposal only; no source edit, migration, exhaustive domain proof, wire compatibility, or Flutter state-management choice. |
| `prevent-regression` | `pattern.md`, `proposal.md`, staged guard/test files, `host-wiring.diff`, `verification.json` show good Dart passes and a reverted `String` field fails. | SHA-bound accepted enum proposal and a dependency-free project-owned direct type-guard script with good/bad disposable trees. | Existing equivalent guard is detected and reported without staging a duplicate. | No accepted review, stale hashes, private/generated/external owner, unavailable direct guard, dynamic-only checks, or unrelated state finding stages a guard. | Host-safe analyze/format/direct guard/smoke on good; bad fails specifically because of the staged guard, with zero audited-host writes. | Stages but never installs; exact reviewed field only, not a universal lint or serialization/runtime invariant. |
...
1. A fresh offline cache does not currently prove availability of the exact
   analyzer closure. Its copied tool package must define locked/offline
   distribution/preflight without modifying the audited host or using ambient
   network. Until then D2/D3/D8 stay pending even though the warmed cache works.
...
4. Pub workspaces, conditional imports/exports, parts, augmentations, code
   ...
   stop at the selected pubspec-only plain-Dart configuration or an explicitly
   supplied pre-existing package configuration.
5. `package:test` is host/closure-owned and `dart test` may trigger Pub
   resolution. It is not part of D0; never execute it in an audited host or
   before locked/offline setup of a closure-owned/disposable fixture.
...
2. Implement D0 only and freeze the dependency-free pubspec-only fixture,
   doctor output, exact SDK range, host-safe zero-write native matrix, roles,
   hashes, and lifecycle. Separately define the closure-owned analyzer package's
   locked/offline acquisition contract.
