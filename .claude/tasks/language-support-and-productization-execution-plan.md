...
Status: active
Current phase: P7/F2 Cohort A, wave A3 Swift reproducibility repair; C#/.NET remains last
Last updated: 2026-07-23 (PHP/Ruby A3 accepted locally; Swift root replay timed out)
...
| A3 — PHP/Ruby/Swift semantic read-only | `repair_swift` | `4e3aa41`; `codex/f2-php-semantic`, `codex/f2-ruby-semantic`, `codex/f2-swift-semantic` | PHP `42f6236`; Ruby `b8f82cf` + missing ignored-fixture repair `10015fb`; Swift candidate `3e13061` not yet accepted | PHP A3 `3 passed` plus preserved `112 passed`; Ruby initially failed on an uncommitted ignored build-role fixture, then A3 `14 passed` and preserved `63 passed`; PHP/Ruby shared projection/router surface `94 passed`; Swift copied root replay exceeded 360 seconds in SourceKit-LSP and remains pending repair | Project/configuration identity and explicit dynamic/unresolved boundaries; Ruby map remains supported/runtime-partial; Swift must have bounded cold/repeated execution before publication |
...
|---|---|---|---|---|---|---|
| PHP expansion | `family_expansion_active` | PHP 8.4.2/Composer 2.4.0 proven | 17 supported, 5 pending implementation; not complete under full-language criteria | A3 capability truth prepared; committed installed replay pending | Pilot plus project/lexical, syntax, and Composer-semantic family packets | A1 `35e957b`; A2 `682a4df`; A3 `42f6236`; publication pending |
...
| Ruby | `family_expansion_active` | Homebrew Ruby 3.4.1, RubyGems 3.6.2, and Bundler 2.6.2 pass direct execution smoke | 16 supported, 6 pending implementation; not complete | A3 capability truth prepared; committed installed replay pending; runtime dynamic completeness stays explicit | Spine/preflight plus map, project/lexical, syntax, and RBS-semantic family packets | A1 through `debbfb2`; A2 `c4b2036`; A3 through `10015fb`; publication pending |
...
| Swift | `family_expansion_active` | SwiftPM-only profile/doctor/inventory and restrictive native fixture proven; SwiftSyntax and native test modules remain unavailable under CLT-only setup | 12 supported, 10 pending implementation; A3 remains unaccepted after root SourceKit timeout | A1/A2 capability truth remains published; A3 repair active | Pilot plus project/lexical, syntax, and candidate semantic packets | A1 `105c7a2`; A2 `f8cc3a4`; A3 candidate `3e13061` |
