...
| B0 — C cohort foundation | `in_progress` | `38b9549`; root plus disjoint C-local lanes | — | C inventory completed against the existing Clang/Make/compile-database spine | Freeze one C-local fact boundary and open only C implementation files; no shared platform |
| B1 — C lexical/filesystem and syntax | `pending_b0` | TBD; up to three C-local lanes | — | — | Nine independent final outcomes and copied closures pass |
| B2 — C semantic read-only | `pending_b1` | TBD; up to three C-local lanes | — | — | Five bounded direct/static outcomes pass; dynamic/function-pointer limits remain explicit |
| B3 — C proposal/guard | `pending_b2` | TBD; up to three accepted-evidence consumers | — | — | Five downstream outcomes validate accepted evidence and preserve human authority |
| B4 — C mutation/publication | `pending_b3` | TBD; serial | — | — | C reaches maximum honest coverage and its installed route passes |
