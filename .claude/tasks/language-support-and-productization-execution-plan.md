...
Status: active
Current phase: P7/F3 Kotlin B7 ready; C, C++, and Ruby are 22/22; Kotlin then C# then maximum-feasible Swift is the fixed remaining order
Last updated: 2026-07-23 (C++ published at 22/22 with native/copied and installed routing boundaries)
...
| P7 | Every chosen language reaches complete, value-tested language-level support | `active_kotlin` | — |
...
| F3 | Complete C, then C++, then Kotlin | `kotlin_ready` | Finish one language before the next; up to three disjoint same-language lanes, with shared publication serial | C and C++ are complete at 22/22 under separate truth; Kotlin may now start from its own project/tool boundary |
...
| B7 — Kotlin spine and lexical/syntax | `in_progress` | `a1dad90`; `codex/kotlin-{lexical-syntax,semantic,move-path}` | spine `7095347`; shared profile integration `a1dad90`; cohorts active | Kotlin/JVM 2.4.10/JDK 17 direct build, test, smoke, copied closure, and honest no-Analysis-API boundary pass | Offline project boundary and pinned syntax provider pass before semantic claims |
| B8 — Kotlin semantic/proposal/mutation publication | `pending_b7` | semantic feasibility lane active; consumers remain gated on its result | — | — | Kotlin reaches maximum honest coverage; unavailable compiler semantics stay explicit |
| C0 — C# spine/P5 | `pending_b8` | TBD; serial | — | — | .NET/Roslyn project, restore/cache, source-role, and native command boundaries frozen |
...
| Kotlin | `implementation_active_unpublished` | Kotlin/JVM 2.4.10 and JDK 17 direct compiler path proven; Gradle intentionally excluded; no standalone Analysis API ships | Spine complete; all 22 skill rows remain pending while lexical/syntax, semantic-feasibility, and move cohorts run | Profile/inventory integrated; matrix/router publication waits for skill outcomes | Kotlin spine packet plus active cohort packets | spine `7095347`; profile integration `a1dad90` |
