...
Status: active
Current phase: P7/F2C AC1 migration vertical slice is in progress by owner request; Ruby A5 resumes after the bounded trial; PHP is 22/22; Swift A3 needs a different semantic foundation; C#/.NET remains last
Last updated: 2026-07-23 (AC1 versioned host-state migration vertical slice started)
...
| F2C | Close the decision-propagation and project-coherence gaps reproduced in engineering-skills and host-a | `in_progress_migration_slice` | Root owns the contract and integration; up to three fresh non-context read-only inventories may audit decision impact, artifact homes, and host drift | AC1 criteria pass: one authoritative state-home decision, explicit propagation obligations, a versioned/reversible host-state migration path, diff-honest cleanup, register-or-exempt subsystem coverage, and durable deferred-debt projection |
...
| AC1 — Decision propagation and project coherence | `in_progress_migration_slice` | owner-authorized trial starts from `6344816`; root/exclusive shared implementation; Ruby A5 remains ready and resumes after this bounded slice | — | — | The migration registry/runner, first state-home migration, real consumers, and disposable-host acceptance tests pass without introducing a package manager |
