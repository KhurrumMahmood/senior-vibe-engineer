...
| B4 — C mutation/publication | `done` | serial C move plus `codex/c-publish-final` | move `e95f222` / packet `2e2836e`; publication in this unit | C reaches 22/22 supported with closure modes, matrix/catalog, and installed routing | Maximum honest bounded C coverage; C++ remains separate |
| B5 — C++ foundation and lexical/syntax | `pending_b4` | TBD; C++-local lanes only | — | — | C++ identity, template/overload, header, ODR, and ABI limits remain separate from C |
| B6 — C++ semantic/proposal/mutation publication | `pending_b5` | TBD; read-only lanes then serial mutation | — | — | C++ reaches maximum honest coverage and its installed route passes |
