...
Status: active
Current phase: P7/F4 C# spine/P5 ready; C, C++, Ruby, and Kotlin are 22/22; C# then maximum-feasible Swift is the fixed remaining order
Last updated: 2026-07-24 (Kotlin completed at 22/22 with bounded native/copied outcomes and publication truth)
...
| F3 | Complete C, then C++, then Kotlin | `done` | Finish one language before the next; up to three disjoint same-language lanes, with shared publication serial | C, C++, and Kotlin are complete at 22/22 under separate bounded truth; Kotlin's final structure consumers are `2c35b98` and the generated publication surfaces are current |
| F4 | Complete C#/.NET last | `ready` | One spine, then disjoint read-only family lanes only where ownership is proven; mutation serial | Roslyn/project facts, all 22 jobs, copied closures, native build/test/analyze, and installed routing pass |
...
| B8 — Kotlin semantic/proposal/mutation publication | `done` | serial integrations through `2c35b98`; shared publication owned by root | semantic `bc08603`; move `5c4a97b`; comment `9a4ac4c`; state/guard `651f8c7`; map `0c9c4c5`; unification `9471914`; structure `2c35b98` | Structure closure `1 passed`; coverage `4 passed`; router recommendations `29 passed`; catalog and matrix freshness pass before the publication commit | Kotlin reaches 22 supported/0 pending with compiler semantics pinned to Kotlin 2.4.10/JDK 17 and no Gradle, Analysis API, framework, reflection, dynamic-dispatch, or whole-language claim |
| C0 — C# spine/P5 | `ready` | TBD; serial | — | — | .NET/Roslyn project, restore/cache, source-role, and native command boundaries frozen |
...
| Kotlin | `complete` | Kotlin/JVM 2.4.10 and JDK 17.0.12 direct compiler path proven; Gradle intentionally excluded; no standalone Analysis API ships | 22 supported, 0 partial, 0 pending, 0 unsupported under bounded-contract semantics | All Kotlin rows, closure modes, aliases, and three router handoffs published; external-library structure closures remain explicit | Spine, lexical/syntax, semantic, comment/map, state/guard, unification, structure-proposal, and transactional move packets | spine `7095347`; profile `a1dad90`; lexical/syntax `28a49e8`; semantic `bc08603`; move `5c4a97b`; downstream through structure `2c35b98` |
