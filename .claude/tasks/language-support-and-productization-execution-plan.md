...
Status: active
Current phase: P7/F4 C# at 20/22; the separate structure lane owns the final two outcomes before C3 publication can close
Last updated: 2026-07-24 (C# C0/C1 evidence and three C2 consumers integrated; shared truth is a 20-supported/2-pending checkpoint)
...
| P7 | Every chosen language reaches complete, value-tested language-level support | `active_csharp` | — |
...
| F4 | Complete C#/.NET last | `active_20_of_22` | One spine, then disjoint read-only family lanes only where ownership is proven; mutation serial | Twenty bounded outcomes are integrated; the two structure proposals and final committed installed replay remain open |
...
| C2 — C# downstream accepted-evidence consumers | `active_structure_pending` | accepted-evidence lanes; separate structure lane remains | enum/guard/unification `e050407`; structure pending | Three content-addressed consumers preserve exact artifacts, human verdicts, and no-mutation authority; `propose-boundary` and `propose-folder-reorganization` have no accepted C# implementation evidence | Do not promote either structure row from pending or claim C2 complete until the separate lane passes copied/native final artifacts |
| C3 — C# mutation and publication | `incomplete_pending_c2` | serial | move `d02bb80`; publication pending | The accepted source-location move passes, but shared truth remains exactly 20 supported/2 pending and the committed installed replay has not run | All 22 C# jobs must close before final publication or the chosen-language release gate starts |
...
| Swift | `family_expansion_active_tool_gap` | SwiftPM-only profile/doctor/inventory and restrictive native fixture proven; SwiftSyntax and native test modules remain unavailable under CLT-only setup | 12 supported, 10 pending implementation; A3 remains unaccepted | A1/A2 capability truth remains published; ML-026 requires a different semantic foundation | Pilot plus project/lexical and syntax packets; rejected semantic candidate is branch-only | A1 `105c7a2`; A2 `f8cc3a4`; A3 candidate `3e13061` removed from main by `20851c2` |
