...
Status: active
Current phase: P7/F3 C lexical/filesystem batch; Ruby is 22/22; C then C++ then Kotlin then C# then maximum-feasible Swift is the fixed remaining order
Last updated: 2026-07-23 (Ruby A5 published and installed replay passed; C started)
...
| P7 | Every chosen language reaches complete, value-tested language-level support | `active_c` | — |
...
| F3 | Complete C, then C++, then Kotlin | `c_in_progress` | Finish one language before the next; up to three disjoint same-language lanes, with shared publication serial | Separate C/C++ truth retained; Kotlin project boundary proven; all three languages reach maximum honest bounded support before C# starts |
...
| B6 — C++ semantic/proposal/mutation publication | `in_progress` | `082f4a0`; `codex/cpp-semantic-proposal`, `codex/cpp-structure-proposals`, `codex/cpp-move-path` | semantic `a8949c4`; downstream pending | State/guard/unification, structure proposals, and move are disjoint active lanes | C++ reaches maximum honest coverage and its installed route passes |
| B7 — Kotlin spine and lexical/syntax | `pending_b6` | TBD; root foundation then Kotlin-local lanes | — | — | Offline project boundary and pinned syntax provider pass before semantic claims |
...
| C++ | `implementation_active_unpublished` | Separate ISO C++20 profile; Apple Clang/clangd 21 and Make proven; trustworthy current complete C++20 compile DB required | 16 implemented and value-tested, 6 downstream/mutation outcomes active; published matrix remains 2 supported until B6 closes | Existing pilot route remains truthful; full publication waits for all 22 outcomes | Spine plus current lexical/syntax and semantic packets; downstream packets active | prior pilot through `7f1c6b3`; current lexical/syntax `0b61858`; semantic `a8949c4`; spine repair `082f4a0` |
