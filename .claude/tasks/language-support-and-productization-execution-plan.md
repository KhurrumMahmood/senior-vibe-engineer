...
Status: active
Current phase: P7 Swift implemented at 22/22; publication replay pending
Last updated: 2026-07-24 (Swift compiler-AST families and capability publication candidate completed; committed installed-router replay remains the final gate)
...
| P7 | Every chosen language reaches complete, value-tested language-level support | `done` | Swift compiler-AST families through `f03cf8a`; publication in this unit |
...
| A7 — Swift full capability publication | `in_progress` | serial shared publication from `f03cf8a` | implementation evidence through `f03cf8a`; publication candidate in this unit | Coverage sentinel, generated catalog/matrix, metadata, release consistency, and exact helper closure gates pass; committed installed-router replay is the remaining completion gate | Candidate truth is 22 supported/0 pending with 19 external-library and three stock-selected-install closures; each of the ten newly promoted skills carries its required `_swift-semantic-readonly` helper closure outside the checkout |
...
| Swift full publication | `.claude/tasks/swift-language-coverage.json`; `.claude/tasks/multilanguage-skill-matrix.json`; `.claude/skills/which-skill/catalog.json`; `tests/test_swift_coverage_matrix.py` | Current truth is 22 supported/0 pending with 19 external-library and three stock-selected-install closures. The ten newly promoted skills expose their provider plus accepted-evidence or structure helper files through generated `language_helpers`, so selected execution can materialize the closure outside a repository checkout. | publication in this unit; committed installed-router replay is the final gate |
...
|---|---|---|---|
| 2026-07-24 | P7/F2 A6-A7 | Swift compiler-AST replacement and full 22/22 publication completed | The historical SourceKit-LSP candidate and failure entries remain intact. Revisions `c5addab`, `f03cf8a`, `2d411cb`, and `50dfd13` instead prove bounded compiler-AST semantic facts and all ten downstream final outcomes. Current coverage has 22 supported/0 pending with exactly 19 external-library and three stock-selected-install closures. The generated manifest carries `_swift-semantic-readonly` plus accepted-evidence or structure helpers for each newly selected skill. Swift coverage/spine, matrix, and release sentinels pass `9`; metadata is `76/76`; catalog/matrix drift, PyCompile, Ruff, and diff checks pass. Commit this unit, then run the cloned installed-router journey from that committed revision as the final gate. |
