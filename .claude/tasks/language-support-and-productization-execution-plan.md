...
| Dart | `spine_complete_family_expansion_active` | Dependency-free Dart SDK 3.12.2 profile, doctor, `.dart` inventory, analyze/format/direct-test/direct-smoke, copied runtime, and zero-write boundaries proven; Flutter remains separate | 0 supported, 22 pending implementation; no skill capability claimed | Pending-only capability column published in current slice | Spine baseline plus `.claude/tasks/multilanguage-learnings/dart-contract-map.md` | spine `f4a5eab`; contract `8e16c5c`, correction `ad6f277` |
