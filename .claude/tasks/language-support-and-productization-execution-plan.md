...
| A5 — Ruby mutation and Cohort A publication | `done` | `5d0608c`; serial | implementation `e727449`; publication `38b9549` | Ruby focused/publication/installed replay `28 passed`; preserved move families `98 passed, 1 skipped`; catalog/matrix and hooks pass | Ruby is 22/22 with preview authority, rollback, exact after-tree, native checks, and a stock-selected copied closure; Swift remains separately tracked under ML-026 |
| A6 — Swift compiler-AST semantic and accepted-evidence families | `done` | serial Swift maximum-feasible pass | semantic/state/unification `c5addab`, `f03cf8a`, `2d411cb`; structure `50dfd13` | Compiler-AST fact production, five read-only consumers, state proposal/guard, shadow unification, and two structure proposals reach their bounded native and copied final artifacts | The earlier SourceKit-LSP failure remains historical evidence; compiler-resolved direct facts replace that transport without claiming whole-program, runtime-dispatch, framework, generated-input, ABI, or mutation authority |
| A7 — Swift full capability publication | `done` | serial shared publication from `f03cf8a` | publication `da51a8d` | Coverage sentinel, generated catalog/matrix, metadata, release consistency, and exact helper closure gates pass; committed installed-router replay passes `59` tests | Swift reaches 22 supported/0 pending with 19 external-library and three stock-selected-install closures; each of the ten newly promoted skills carries its required `_swift-semantic-readonly` helper closure outside the checkout and missing helpers fail closed |
...
| B0 — C cohort foundation | `done` | `38b9549`; root plus disjoint C-local lanes | spine `56707fe`; runtime closure repair `e86c210` | C inventory and trustworthy Clang/Make/compile-database boundary pass | C-local fact boundaries stayed language-local; no shared platform |
| B1 — C lexical/filesystem and syntax | `done` | C-local lanes integrated through `2481fd8` | lexical `2a93ee1`; syntax `2481fd8` | Five lexical/filesystem and four syntax outcomes reach copied final artifacts | Nine independent final outcomes and copied closures pass |
| B2 — C semantic read-only | `done` | C-local lane integrated | semantic `80c8941` | Five conservative direct/static outcomes plus the accepted map pass | Dynamic/function-pointer limits remain explicit |
| B3 — C proposal/guard | `done` | accepted-evidence C-local lanes integrated | state proposal/guard `5a41abd`; unification `7772fd7`; structure proposals `1dd2be7` | Five downstream outcomes validate accepted evidence, lifecycle replacement, and human authority | Proposal and guard claims remain read-only/staged and bounded |
| B4 — C mutation/publication | `done` | serial C move plus retired `codex/c-publish-final` | move `e95f222` / packet `2e2836e`; publication `f09f227` | C reaches 22/22 supported with closure modes, matrix/catalog, and installed routing | Maximum honest bounded C coverage; C++ remains separate |
| B5 — C++ foundation and lexical/syntax | `done` | `f09f227`; retired `codex/cpp-lexical-syntax`, semantic lane retained for B6 | lexical/syntax `0b61858`; semantic foundation `a8949c4`; runtime manifest repair `082f4a0` | Nine lexical/syntax and five semantic outcomes pass focused/copied/native checks; the repaired five-test C++ spine passes | C++ identity, template/overload, header, ODR, and ABI limits remain separate from C |
| B6 — C++ semantic/proposal/mutation publication | `done` | `082f4a0`; retained implementation branches; serial publication | semantic `a8949c4`; state/guard/unification `33bd2db`; move `0022241`; structure `c7c9b55`; publication `18c5ab6` | All eight focused/preserved C++ suites pass `64`; catalog/matrix plus publication/conformance pass `74`; committed installed-router replay passes `59` | C++ is 22/22 with exact closure modes, bounded native claims, and a formerly pending adapter route |
| B7 — Kotlin spine and lexical/syntax | `done` | `a1dad90`; retired cohort worktrees, retained branches | spine `7095347`; shared profile `a1dad90`; lexical/syntax `28a49e8` | Kotlin/JVM 2.4.10 on JDK 17.0.12 direct build, test, smoke, copied closure, and honest no-Analysis-API boundary pass | Offline project boundary and pinned syntax provider pass before semantic claims |
| B8 — Kotlin semantic/proposal/mutation publication | `done` | serial integrations through `2c35b98`; shared publication owned by root | semantic `bc08603`; move `5c4a97b`; comment `9a4ac4c`; state/guard `651f8c7`; map `0c9c4c5`; unification `9471914`; structure `2c35b98`; publication `52fc489` | Structure closure `1 passed`; coverage `4 passed`; router/publication suite `192 passed`; catalog and matrix freshness pass; committed installed-router replay `59 passed` | Kotlin reaches 22 supported/0 pending with compiler semantics pinned to Kotlin 2.4.10/JDK 17 and no Gradle, Analysis API, framework, reflection, dynamic-dispatch, or whole-language claim |
...
- [x] Dogfood the first real migration on the accepted state-home decision:
...
|---|---|---|---|
| Plan/review | This section plus **AC1 plan review record** below; commit hooks | Reviewed/repaired; hook suite passed | `0bc7df4` |

#### AC1-S1 — Finding-producer scope contract

Acceptance:

- [x] A machine-readable inventory accounts for every top-level `find-*` skill
...
- [x] Every producer declares supported modes from `diff-lines`,
  ...
  or `not-applicable`.
- [x] The contract explains how legitimate obligations outside a changed hunk
...
- [x] Existing project-wide and per-skill roots/ignores are either consumed or
...
- [x] Scope precedence is explicit: safety exclusions and project containment
...
- [x] The initial classifications receive a bounded human review by behavior
  family (line-local, symbol/file semantic, multi-site/history, project/global),
  rather than 76 bespoke skill-design exercises.
- [x] Contributor guidance explains when to add or revise a scope-contract row,
...
| Check | Command/artifact | Result | Revision |
|---|---|---|---|
| Inventory completeness and schema | `.venv/bin/python scripts/check_scan_scope_contracts.py`; `.venv/bin/python -m pytest -q tests/test_scan_scope_contracts.py tests/test_scope.py` | `OK (30 find-* skills)`; `38 passed` | `1340c10` |
| Commit guard and style | `.venv/bin/pre-commit run scan-scope-contract --all-files`; `.venv/bin/ruff check scripts/check_scan_scope_contracts.py tests/test_scan_scope_contracts.py` | Passed; passed | `1340c10` |
...
|---|---|---|---|---|---|---|
| PHP expansion | `complete` | PHP 8.4.2/Composer 2.4.0 proven | 22 supported, zero pending implementation | A4 publication `2e7cc75`; committed installed replay `58 passed` | Pilot plus project/lexical, syntax, Composer-semantic, and accepted-evidence proposal/guard packets | A1 `35e957b`; A2 `682a4df`; A3 `42f6236`; A4 `0b38efa` |
...
| Ruby | `complete` | Homebrew Ruby 3.4.1, RubyGems 3.6.2, and Bundler 2.6.2 pass direct execution smoke | 22 supported, zero pending | A5 publication `38b9549`; committed installed route passes; runtime dynamic completeness stays explicit | Spine/preflight plus map, project/lexical, syntax, RBS-semantic, accepted-evidence proposal/guard, and transactional move packets | A1 through `debbfb2`; A2 `c4b2036`; A3 through `10015fb`; A4 `98c80aa`; A5 `e727449`/`38b9549` |
...
| Swift | `complete` | SwiftPM profile/doctor/inventory plus Apple Swift 6.3.3 compiler-AST and Swift 6.3.0 formatting authority proven; no SwiftSyntax, SourceKit-LSP, or Xcode dependency is required | 22 supported, 0 partial, 0 pending, 0 unsupported under bounded-contract semantics | All 22 rows and exact closure modes are published: 19 external-library and three stock-selected-install; the ten newly promoted rows carry explicit portable helper closures | Pilot, project/lexical, syntax, compiler-AST semantic/state, accepted-evidence proposal/guard/unification, structure-proposal, and transactional move packets; the rejected SourceKit candidate remains historical evidence | A1 `105c7a2`; A2 `f8cc3a4`; semantic `c5addab`; state `f03cf8a`; semantic/unification `2d411cb`; structure `50dfd13`; publication `da51a8d` |
...
| Kotlin | `complete` | Kotlin/JVM 2.4.10 and JDK 17.0.12 direct compiler path proven; Gradle intentionally excluded; no standalone Analysis API ships | 22 supported, 0 partial, 0 pending, 0 unsupported under bounded-contract semantics | Publication `52fc489`; all Kotlin rows, closure modes, aliases, and three committed router handoffs pass; external-library structure closures remain explicit | Spine, lexical/syntax, semantic, comment/map, state/guard, unification, structure-proposal, and transactional move packets | spine `7095347`; profile `a1dad90`; lexical/syntax `28a49e8`; semantic `bc08603`; move `5c4a97b`; downstream through structure `2c35b98`; publication `52fc489` |
| C | `complete` | Apple Clang/clangd 21 plus Make 3.81 proven; `.c`/`.i` only; trustworthy current complete C17 compile DB required; owned headers admitted through dependency closure | 22 supported, 0 partial, 0 pending, 0 unsupported under bounded-contract semantics | All C rows and closure modes published; formerly pending adapter route now reaches its external-library handoff | Pilot plus lexical/filesystem, syntax, semantic, state proposal/guard, structure proposal, unification, and move packets | pilot through `79d8a27`; lexical `2a93ee1`; syntax `2481fd8`; semantic `80c8941`; downstream through `1dd2be7` |
| C++ | `complete` | Separate ISO C++20 profile; Apple Clang/clangd 21, Make, and nm proven; trustworthy current complete C++20 compile DB and compiler-owned headers required | 22 supported, 0 partial, 0 pending, 0 unsupported under bounded-contract semantics | All C++ rows and closure modes published; formerly pending adapter route reaches its external-library handoff | Spine, comment/map, lexical/syntax, semantic, state/proposal/guard/unification, structure-proposal, and transactional move packets | pilot through `7f1c6b3`; lexical/syntax `0b61858`; semantic `a8949c4`; state `33bd2db`; move `0022241`; structure `c7c9b55` |
...
| C | Trustworthy host or externally generated `compile_commands.json`; no new compiler required | CMake, Meson, clang-tidy, clang-format, IWYU, cppcheck, capture tools such as Bear | Apple Clang/clangd 21 and Make are available. Owner-authorized real-repo validation installed Homebrew CMake 4.4.0 and Meson 1.11.2; Meson installed Ninja 1.13.2 and Python 3.14.6 and upgraded Homebrew SQLite to 3.53.4. The C runner accepts the standard external Meson database without touching the source checkout. Cleanup may run `brew uninstall cmake meson`, but remove Ninja/Python/SQLite only after `brew uses --installed` and unrelated-use review. |
| C++ | Separate C++ profile plus trustworthy compile commands; no new compiler required | Same Clang ecosystem as C, selected standard library/build-system tools | Apple Clang/clangd 21 and Make are available. The same Homebrew CMake/Meson installation above generated an external standard CMake database for the real-repo proof; incomplete target coverage retains useful compiler-owned syntax facts as partial. Do not combine C and C++ capability truth. |
...
| Swift compiler-AST semantic and downstream families | `.claude/tasks/multilanguage-learnings/swift-semantic-a3.md`; `.claude/tasks/multilanguage-learnings/swift-state-proposal-guard.md`; `.claude/tasks/multilanguage-learnings/swift-unify-shadows.md`; `.claude/tasks/multilanguage-learnings/swift-structure-proposals.md` | A current complete `swift-semantic-facts-v2` pack supplies bounded compiler-resolved direct facts. Five read-only consumers and five accepted-evidence proposal/guard consumers reach independent final artifacts while retaining dynamic/protocol dispatch, reflection, generated/conditional inputs, external modules, frameworks, ABI, runtime-domain, and mutation non-claims. | semantic `c5addab`; state `f03cf8a`; semantic/unification `2d411cb`; structure `50dfd13` |
| Swift full publication | `.claude/tasks/swift-language-coverage.json`; `.claude/tasks/multilanguage-skill-matrix.json`; `.claude/skills/which-skill/catalog.json`; `tests/test_swift_coverage_matrix.py` | Current truth is 22 supported/0 pending with 19 external-library and three stock-selected-install closures. The ten newly promoted skills expose their provider plus accepted-evidence or structure helper files through generated `language_helpers`, so selected execution can materialize the closure outside a repository checkout; missing helpers fail closed. | publication `da51a8d`; committed installed-router replay `59 passed in 20.95s` |
...
| C cohorts and publication | Focused C family suites; `tests/test_c_pilot_spine.py`; `tests/test_multilanguage_expansion_matrix.py`; `tests/test_installed_routers.py`; C learning packets | The historical frozen spine remains 22 pending at capture and the two-skill pilot remains preserved. Current lexical/filesystem, syntax, semantic, accepted-evidence, guard, proposal, unification, and transactional move outcomes pass; publication closes 22/22 with explicit external-library versus stock-selected closure modes and a formerly pending adapter route. | pilot through `79d8a27`; lexical `2a93ee1`; syntax `2481fd8`; semantic `80c8941`; downstream through `1dd2be7`; publication in this unit |
| C++ cohorts and publication | Focused C++ family suites; frozen `.claude/tasks/p7-baseline/cpp-pilot-baseline.json`; current `.claude/tasks/cpp-language-coverage.json`; matrix/catalog/router/conformance suites | The historical baseline remains 22 pending at capture. Current comment/map, lexical/syntax, semantic, state/proposal/guard/unification, structure-proposal, and transactional move outcomes close 22/22 under exact C++20 compile-database, compiler-owned-header, namespace/signature/overload, human-authority, and explicit ODR/ABI/specialization/dispatch/variant limits. Full C++ replay passes `64`; publication/conformance passes `74`; committed installed routing passes. | pilot through `7f1c6b3`; lexical/syntax `0b61858`; semantic `a8949c4`; downstream `33bd2db`; move `0022241`; structure `c7c9b55`; publication in this unit |
...
|---|---|---|---|
| 2026-07-24 | P7/F2 A6-A7 | Swift compiler-AST replacement and full 22/22 publication completed | The historical SourceKit-LSP candidate and failure entries remain intact. Revisions `c5addab`, `f03cf8a`, `2d411cb`, and `50dfd13` instead prove bounded compiler-AST semantic facts and all ten downstream final outcomes. Publication `da51a8d` records 22 supported/0 pending with exactly 19 external-library and three stock-selected-install closures. The generated manifest carries `_swift-semantic-readonly` plus accepted-evidence or structure helpers for each newly selected skill, and routing fails closed if one is missing. Swift coverage/spine, matrix, and release sentinels pass `9`; which-skill regression coverage passes `116`; metadata is `76/76`; catalog/matrix drift, PyCompile, Ruff, diff checks, and commit hooks pass. The committed cloned installed-router replay passes `59` tests in `20.95s`. |
...
| 2026-07-23 | P7/F3 B6 | C++ completed and published at 22/22 | Structure proposals `c7c9b55` close the final two accepted-evidence consumers without source authority. The final coverage records 17 external-library and five stock-selected closures; exact namespace/signature/overload identity and compiler-owned headers stay explicit, while ODR, ABI, specializations, dynamic dispatch, external consumers, and alternate variants remain non-claims. All focused/preserved C++ suites pass `64`; catalog/matrix and publication/conformance pass `74`; committed installed routing reaches the formerly pending adapter handoff. Begin Kotlin B7 only after this committed closeout. |
| 2026-07-23 | P7/F3 B6 | C++ transactional source move integrated | Source commit `df44de5`, integrated on main as `0022241`, adds one stock-selected, content-addressed same-filename implementation-unit move with exact include/Make edits, compiler-owned dependency impact, native test/smoke, global defined-symbol equality, exact after-tree, rollback, and copied single-file closure. Current coverage is provisionally 20 supported/2 pending; wait for the two structure proposals before final generated publication. |
| 2026-07-23 | P7/F3 B6 | C++ state/guard/unification integrated; final three lanes remain | `33bd2db` adds content-addressed exact-field enum proposal, staged type guard, and read-only shadow-unification outcomes on the accepted C++ semantic facts. Current coverage is provisionally 19 supported/3 pending with external-library closure truth; do not regenerate the final matrix/catalog or publish routing until the two structure proposals and `move-path` revisions integrate and all 22 native/copied boundaries replay. |
| 2026-07-23 | P7/F2 A5 | Ruby mutation and publication completed | `e727449` adds one self-contained Prism-attributed, content-addressed Ruby module mover; `38b9549` publishes Ruby at 22/22 with a stock-selected closure and committed router truth. Focused/publication/installed replay passes `28`; preserved Go/Java/PHP/Swift/Rust/Dart/generic move families pass `98 passed, 1 skipped`; catalog/matrix, Ruff, artifact drift, and commit hooks pass. Begin C from its existing Clang/Make/compile-database spine with no infrastructure detour. |
...
| 2026-07-23 | P7/F2 A3 | PHP/Ruby/Swift semantic read-only wave opened | Three fresh non-context language worktrees start from exact A2 closeout `4e3aa41`. Each owns the five pending read-only semantic outcomes: dormant code, implicit state, incomplete sweep, semantic duplication, and rename assessment. Existing accepted maps are preserved. Workers may share only a measured language-local fact boundary and must retain honest dynamic/index/configuration limits; root retains shared skill prose, coverage, projections, routers, docs, and serial publication. |
| 2026-07-23 | P7/F2 A3 | PHP/Ruby integrated; Swift held for reproducibility repair | PHP `42f6236` passes `3` copied final-outcome tests and `112` preserved PHP tests. Ruby `b8f82cf` exposed one ignored build-role fixture missing from its commit; repair `10015fb` restores the frozen manifest and passes `14` A3 plus `63` preserved tests. PHP/Ruby publication projections and routing pass `94`. Swift candidate `3e13061` is retained but not published: root's copied union exceeded 360 seconds and a one-query probe remained blocked in SourceKit-LSP `documentSymbol` after 137 seconds. Repair the bounded protocol path or keep all five Swift rows pending. |
| 2026-07-23 | P7/F2 A3 | PHP/Ruby A3 published; Swift candidate removed from main | Publication `c7cee0b` and installed expectation `c00df4e` pass the shared 94-test projection/router surface and the committed 58-test installed-router journey. Swift diagnosis found that initialization succeeded but the first semantic request remained unanswered after explicit indexing, forced SwiftPM workspace selection, and a readiness wait. Main removal `20851c2` avoids shipping 4,535 lines of unpublished nonfunctional code; the research branch remains available and ML-026 requires a different verified foundation. Open PHP/Ruby A4 independently. |
| 2026-07-23 | P7/F2 A4 | PHP/Ruby proposal/guard lanes opened independently | From exact base `a3855b0`, `codex/f2-php-proposal-guard` and `codex/f2-ruby-proposal-guard` each own the five pending accepted-evidence consumers: enum extraction, regression guard, architecture boundary proposal, folder reorganization proposal, and shadow unification. Root retains shared publication. Swift A4 remains closed until ML-026 supplies accepted A3 evidence. |
| 2026-07-23 | P7/F2 A4 | PHP/Ruby proposal/guard implementations integrated | PHP `0b38efa` passes `9` root tests in 131.25 seconds; Ruby `98c80aa` passes `18` root tests in 38.01 seconds despite the worker's miswired local hook. Both consume current hash-bound evidence without redetection, preserve safe-defer outcomes, and pass ML-025. Shared capability generation and router/spine tests pass `94`; conformance passes `6`; metadata reports 76/76; artifact drift passes. Commit publication and replay the installed library before closing A4. |
| 2026-07-23 | P7/F2 A4 | PHP/Ruby proposal/guard wave closed | Publication `2e7cc75` passes all commit hooks. The full release/router surface passes `163`, and the committed external-library/router journey passes `58`. PHP is now 22/22 supported; Ruby is 21/22 with only `move-path` pending. Retire the clean A4 worktrees, retain their branches, and open serial Ruby A5 from this closeout. |
