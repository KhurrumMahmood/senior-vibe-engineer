...
Last updated: 2026-07-24 (Swift publication `100e701` and its committed installed-router replay completed with 59 passing tests)
...
| P7 | Every chosen language reaches complete, value-tested language-level support | `done` | Swift compiler-AST families through `f03cf8a`; publication `100e701` |
...
| Swift full publication | `.claude/tasks/swift-language-coverage.json`; `.claude/tasks/multilanguage-skill-matrix.json`; `.claude/skills/which-skill/catalog.json`; `tests/test_swift_coverage_matrix.py` | Current truth is 22 supported/0 pending with 19 external-library and three stock-selected-install closures. The ten newly promoted skills expose their provider plus accepted-evidence or structure helper files through generated `language_helpers`, so selected execution can materialize the closure outside a repository checkout; missing helpers fail closed. | publication `100e701`; committed installed-router replay `59 passed in 20.40s` |
...
|---|---|---|---|
| 2026-07-24 | P7/F2 A6-A7 | Swift compiler-AST replacement and full 22/22 publication completed | The historical SourceKit-LSP candidate and failure entries remain intact. Revisions `c5addab`, `f03cf8a`, `2d411cb`, and `50dfd13` instead prove bounded compiler-AST semantic facts and all ten downstream final outcomes. Publication `100e701` records 22 supported/0 pending with exactly 19 external-library and three stock-selected-install closures. The generated manifest carries `_swift-semantic-readonly` plus accepted-evidence or structure helpers for each newly selected skill, and routing fails closed if one is missing. Swift coverage/spine, matrix, and release sentinels pass `9`; which-skill regression coverage passes `116`; metadata is `76/76`; catalog/matrix drift, PyCompile, Ruff, diff checks, and commit hooks pass. The committed cloned installed-router replay passes `59` tests in `20.40s`. |
