...
| Rust | `family_batches_active` | rustc/Cargo 1.97.1 plus rust-analyzer, Clippy, and rustfmt installed through rustup | 2 supported (`find-comment-drift`, `move-path`), 1 useful partial (`map-subsystem`), 19 pending implementation; none unsupported; not complete | Schema-v5 column published; semantic publication in current slice | Rust spine and three pilot learning packets | spine `f86f11c`; lexical `c3bcab2`; mutation `4149971`; semantic `f0bba1a`; base publication `8c227b2` |
