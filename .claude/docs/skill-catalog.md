...
| `/find-incomplete-sweep` | Recently introduced N-1-of-N changes: majority call sites carry a new keyword/value shape while a git-older straggler remains at the old shape; also detects referenced concrete placeholders. Detection is trajectory-gated and scout-judged before handoff. | After a multi-site human/AI sweep where one sibling call site may have been missed; use `/find-dormant` for unreferenced code and `/fix-workflow` only after a `forgotten` verdict. |
| `/find-perimeter-gaps` | Significant `(code root × language)` cells with no installed SUSPECT detector declaring executable scan coverage. Accepted generated/vendor blind spots remain visible with reasons. | During `/adapt-project`, before any whole-codebase audit conclusion, and after adding/rescoping language adapters. Makes unsupported language surfaces explicit; it does not find smells inside covered code. |
...
| `/design-it-twice` | Resolves a material design fork by sending the same problem to three independent designers with deliberately different constraints, then synthesizing a comparison and recommendation without authoring the ADR. | When a high-cost fork has multiple defensible answers and one design pass would anchor too early. Hand the result to `/decide`. |
...
| `/repair-skill` | Repairs a skill whose execution-time text cannot be trusted: frame review, claim-verifying scout, declared change spec, no-invention implementation, independent verification, lift probes, and real-host dogfood when execution-heavy. | After a production incident, failed dogfood, or frame review identifies a concrete instruction defect. Use `/plan-skill` when the skill needs redesign rather than bounded repair. |
