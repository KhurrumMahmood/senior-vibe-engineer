...
| `/organize-project-structure` | Repo-wide folder-structure redesign around intuitive top-level ownership, artifact lifecycle, source-material preservation, target topology, and a batched `/move-path` execution plan. | When historical top-level folders such as inputs/outputs/archive dumps make a repo hard to skim, or when a proposed project tree needs review, dry-run movement, and reference-safe execution. Use `/move-path` directly for a known single move. |
