...
| `/find-incomplete-sweep` | Majority-versus-straggler call-site divergence with language-specific resolution and Git-trajectory evidence; distinguishes a likely unfinished multi-site change from recent deliberate asymmetry. Detection-only. | After a parameter, option, constructor, or call shape was changed across several sibling sites and one may have been missed. |
| `/find-perimeter-gaps` | Inverts the detector inventory into a code-root × language coverage matrix and reports significant cells with no declared SUSPECT coverage. Detection-only; accepted blind spots remain explicit. | During project adoption, broad audits, and after adding or rescoping language support. |
...
| `/which-shape` | Strategic router. Reads the bundled `which-shape/shapes.json` registry plus `.engineering/project/` adapter/profile state and recommends an advisory operating loop with first next step, sequence, stop/reassess condition, and exact on-demand handoff. It does not execute the loop. | When the situation is ambiguous at the loop level: unknown project, messy legacy code, broad health question, recurring failure, or unclear feature/refactor/decision shape. |
| `/which-cleanup` | Portable, scope-tiered closeout router (ADR 0023). Resolves explicit paths, the working tree, `--staged`, `--changed-from`, `--commit`, or `--range`; projects one reusable scan request through scanner-specific `diff-lines`, `changed-files`, path, or project scope; and returns eligible on-demand guide/tool closures. It does not execute the checks or escalate into a hidden workflow. | At task/commit closeout: "I changed these files — what cleanup do I owe, scaled to how big the change is?" The change-bounded counterpart to periodic `/triage-debt`; the `task-closeout` shape in `/which-shape` points here. |
...
| `/which-skill` | Tactical recommender. Reads a generated metadata-only catalog, ranks it against a free-text task, and returns one recommendation, a bounded ranked set, or `proceed_directly`. A successful route exposes the exact external-library guide/tool closure and fresh non-context handoff without loading every skill body. | When the right skill isn't obvious, especially at the start of a non-trivial task. Defends against funneling a 1-line fix into heavy planning machinery. |
...
| `/repair-skill` | Evidence-driven repair loop for an existing skill that cannot be trusted from its text alone: frame, verify claims, specify the change, implement without invention, independently verify, and measure lift. | When a dogfood, frame review, incident, or activation-standard gap demonstrates that an existing skill misleads or under-constrains execution. |
| `/engineer-init` | Bootstraps the ecosystem runtime — checks Python >= 3.11, creates `.venv`, installs `requirements.txt` and locked Node tooling when declared, wires pre-commit hooks, and verifies both dependency lanes. Idempotent; `--check` reports status without installing. | Once per clone before invoking any script-backed skill; or when a skill errors out with a missing-module / dependency failure. |
...
Packet-compatible work staged under `.engineering/local/queue/` follows
[`queue-contract.md`](queue-contract.md); `scripts/status.py` projects that
queue without making it another skill-discovery surface.
