...
Freeze the pre-repair skill: `cp -R .claude/skills/<skill>
/tmp/skill-repairs-old/<skill>` (probes need the old condition).
Create `.claude/tasks/skill-repairs/<skill>/`.

Scale gate — pick and state one:
- **Full loop** (doctrine-bearing or execution-heavy skills, or ≥3
  high-severity findings): all stages including dogfood.
- **Light loop** (small prose-only skills, ≤2 findings): Stages 1–5
  plus one probe cell; state what was skipped and why.

### Stage 1 — Frame review (skip if a current one exists)

Dispatch a fresh sub-agent (`general-purpose`) with
`knowledge/frame-review-rubric.md`, the skill's full file list, and
its script contracts. Output:
...
green, plus any skill-local validate command. The implementer does
not commit.
...
defect site, weakest supported tier (haiku), old (frozen copy) vs new
(working tree), fresh blind agents. Judge into `lift-report.md` —
scoring **grounding as well as behavior** (do cited mandates exist?).
A regression at any site blocks close-out until explained or fixed.
...
written on, under hostile-but-realistic conditions (no convention
docs, no venv, detached HEAD, commits forbidden). The executor logs
