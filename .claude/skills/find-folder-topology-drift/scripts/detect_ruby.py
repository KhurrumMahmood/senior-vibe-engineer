#!/usr/bin/env python3
"""Produce Ruby direct-sibling filename topology findings and final artifacts."""
        ...
        findings.append(
            {
                ...
                "recommendation": (
                    "Human triage only; no require/load, Zeitwerk, constant, or import-safe move is implied."
                ),
            }
        )
    return findings
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--ruby-root", required=True)
    ...
    lines = [
        "# Folder-topology drift audit — Ruby",
        "",
        f"**Status:** `{snapshot['status']}`",
        f"**Outcome:** `{outcome}`",
        f"**Target:** `{args.ruby_root}`",
        "",
    ]
    if findings:
        lines.extend(
        ...
        lines.append("\nFindings are lexical filename evidence only; do not move files automatically.")
    elif snapshot["status"] == "complete":
        lines.append("No Ruby direct-sibling prefix cluster met the threshold.")
    else:
