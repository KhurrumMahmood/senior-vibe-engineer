...
argument-hint: "<plan-name> [--spec-id <id>]  (must be ai-docs/plans/<name>.md with status=architected)"
...
- **Python:** `.venv/bin/python` for `scripts/plans.py` and
  `scripts/specs.py` (they use the shared PyYAML-backed frontmatter
  parser from `scripts/_lib/`).
