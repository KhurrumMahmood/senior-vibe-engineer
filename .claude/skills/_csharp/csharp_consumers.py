#!/usr/bin/env python3
"""Independent final-artifact executors for bounded C# source facts."""
...
def adapt_main(argv: list[str] | None = None) -> int:
    parser = _parser("Produce a bounded C# 14 / .NET 10 host adapter.")
    ...
    adapter = {
        ...
        "stack": {
            "languages": ["csharp"] if facts.get("inventory") else [],
            "frameworks": [], "package_managers": [],
            "build_systems": ["direct-csc-manifest"] if project else [],
        },
        "commands": {
            "manifest": ["csharp-project.json"],
        ...
        "source_roots": [{
            ...
            "source_languages": ["csharp"] if sources else [],
        }],
        "boundaries": facts.get("boundaries", {}),
        "standardization": {"cautions": [
            "Observed direct C# inputs do not endorse MSBuild evaluation, source generators, multi-targeting, or a repository layout."
        ]},
        "open_questions": [
    ...
    atomic_text(
        artifacts[2],
        "# Adapt Project Report — C# 14 / .NET 10\n\n"
...
def explain_main(argv: list[str] | None = None) -> int:
    parser = _parser("Render directly spelled C# declarations.")
    ...
    payload = {
        "schema_version": 1, "status": facts["status"], "language": "csharp",
        "target": str(args.target), "analysis": {"csharp": facts},
    ...
    atomic_text(
        ...
        "Direct C# declaration and overload spellings follow; resolved semantics do not.\n\n"
...
def concept_main(argv: list[str] | None = None) -> int:
    parser = _parser("Find strict glossary-backed C# identifier divergence.")
            ...
            for file in facts["files"]:
                ...
                for token in file["identifier_tokens"]:
                    ...
                    findings.append({
                        ...
                        "file": file["file"], "language": "csharp", "term": term,
    ...
    atomic_json(final_json, {
        ...
        "analysis": {"csharp": facts}, "detections_sha256": hash_bytes(jsonl.encode()),
        "findings": findings,
    })
    atomic_json(scan_json, facts)
    lines = [
        "# Concept-divergence scan — C#", "", f"**Status:** `{facts['status']}`",
        f"**Outcome:** `{outcome}`", "",
    ]
...
def duplication_main(argv: list[str] | None = None) -> int:
    parser = _parser("Find exact normalized C# function-body token clones.")
    parser.add_argument("--target", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    args = parser.parse_args(argv)
    root = args.project_root.resolve()
    ...
    for digest, rows in sorted(groups.items()):
        ...
        findings.append({
            "finding_id": f"CSHARP-DUP-{digest[:12].upper()}",
            "detector": "csharp-exact-normalized-function-body-tokens",
    ...
    payload = {
        "schema_version": 1, "status": facts["status"],
        "failure_kind": facts["failure_kind"], "analysis": {"csharp": facts},
    ...
    lines = [
        "# Duplication triage — C#", "", f"**Status:** `{facts['status']}`", "",
...
def folder_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--csharp-root", required=True)
    ...
    clear_artifacts([output, report, final_json, scan_json])
    args.target = Path(args.csharp_root)
    facts, code = _facts(args)
    groups: dict[tuple[str, str], list[str]] = defaultdict(list)
    if facts["status"] == "complete":
        for row in facts["inventory"]:
            path = Path(row["file"])
            if row["role"] != "source" or path.suffix != ".cs":
                continue
    ...
    findings = [{
        "pattern": "flat_prefix_cluster", "language": "csharp", "file": directory,
    ...
    atomic_json(final_json, {
        "schema_version": 1, "status": facts["status"], "outcome": outcome,
        "analysis": {"csharp": facts}, "findings": findings,
    })
    atomic_text(
        report,
        "# Folder-topology drift — C#\n\n"
        f"**Status:** `{facts['status']}`\n**Outcome:** `{outcome}`\n\n"
...
def audit_main(argv: list[str] | None, audit_path: Path) -> int:
    parser = _parser("Audit decision references in C# source comments.")
    parser.add_argument("--target", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    args = parser.parse_args(argv)
    root = args.project_root.resolve()
    ...
    if facts["status"] != "complete":
        atomic_json(artifacts[1], {
            ...
            "analysis": {"csharp": facts}, "references": [], "drift": [],
        })
        ...
        return terminal_return_code(facts, code)
    audit = _module(audit_path, "audit_decisions_csharp_base")
    ...
    references = [{
        "path": file["file"], "line": comment["line"], "language": "csharp",
        "comment_form": comment["form"], "id": match.group(1),
        "resolved": match.group(1) in known,
        "evidence_scope": "csharp-source-comment-token-only",
    ...
    raw = {
        "status": "complete", "failure_kind": "none", "analysis": {"csharp": facts},
    ...
    atomic_text(
        ...
        + "\nC# evidence is source-comment syntax only; runtime applicability is unresolved.\n",
    )
...
def complexity_main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        "status": facts["status"], "failure_kind": facts["failure_kind"],
        "verdict": verdict, "analysis": {"csharp": facts}, "findings": findings,
    }
...
def omnibus_main(argv: list[str] | None = None) -> int:
    ...
    if facts["status"] == "complete":
        for file, functions in sorted(by_file.items()):
            ...
            if len(confirmed) >= 4:
                detections.append({
                    "file": file, "language": "csharp", "cluster_count": len(confirmed),
                    "clusters": confirmed,
                    "evidence_scope": "function-name-cluster-source-syntax-only",
                    "csharp_boundary_note": "top-level/member/extension names can be facets; scout must decide domains",
                })
    ...
    if missing and status == "complete":
        status, failure = "partial", "csharp_scout_evidence_missing"
    payload = {
        "status": status, "failure_kind": failure, "analysis": {"csharp": facts},
...
def standards_main(argv: list[str] | None = None) -> int:
    parser = _parser("Measure configured C# unresolved call-spelling/if syntax coverage.")
    parser.add_argument("--target", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    ...
    for idea in ideas:
        ...
        for pattern in detector.get("paths", []):
            matched.update(
                ...
                if path.is_file() and not path.is_symlink() and path.suffix == ".cs"
            )
    ...
    payload = {
        "status": facts["status"], "failure_kind": facts["failure_kind"],
        "analysis": {"csharp": facts}, "standards": results,
    }
