#!/usr/bin/env python3
"""Write advisory C direct-body complexity leads and final artifacts."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    findings = [
        {
            ...
            "macro_nodes_omitted": function["macro_nodes_omitted"],
            "evidence_scope": "direct-body-syntax-only",
            "summary": "advisory syntax score; measure runtime cost before changing code",
        }
    ...
    status = facts["status"]
    if status != "complete":
        verdict = "incomplete-syntax-evidence"
    else:
        verdict = "measure-first" if findings else "no-hotspots"
    payload = {
        "status": status,
    ...
    lines = [
        ...
        "",
        "Scores are advisory direct-body syntax only; macro-origin AST subtrees are omitted.",
        "",
    ]
    lines.extend(
