#!/usr/bin/env python3
"""Write Ruby direct-call/rescue coverage cells and final reports."""
...
def _terminal(output: Path, facts: dict[str, Any], failure_kind: str) -> None:
    payload = {
        "status": "failed",
    ...
    _atomic(output / "coverage.md", f"# Standard coverage\n\nStatus: `failed`\n\nFailure: `{failure_kind}`\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _terminal(output, {"status": "failed", "failure_kind": "invalid_standards", "files": []}, "invalid_standards")
        return 1
    ...
    for idea in ideas:
        ...
        if detector.get("kind") in {"manual", "skill"}:
            ...
            continue
        if detector.get("kind") != "ast" or detector.get("enclosed_by") != "rescue":
        ...
        for pattern in detector.get("paths", []):
            matched.update(
                ...
                if path.is_file() and path.suffix == ".rb" and not path.is_symlink()
            )
        ...
        gaps = [site for site in sites if "rescue" not in site["enclosures"]]
        ...
        results.append({**base, "status": status, "situation_sites": len(sites), "gap_count": len(gaps), "coverage_percent": coverage, "gaps": gaps})
    payload = {
        "status": facts["status"],
    ...
    for row in results:
        lines.append(f"- `{row['id']}` — `{row['status']}`; {row['situation_sites']} sites, {row['gap_count']} gaps, {row['coverage_percent']}% coverage")
    _atomic(output / "coverage.md", "\n".join(lines) + "\n")
    total_gaps = sum(row["gap_count"] for row in results if row["status"] == "scanned")
    print(f"Ruby Prism syntax: scanned {sum(row['status'] == 'scanned' for row in results)}/{len(results)} standard(s): {total_gaps} coverage gap(s)")
    if code:
        return code
