#!/usr/bin/env python3
"""Carry C syntax candidates through the existing explicit scout gate."""
...
def _valid_scout(scout: dict[str, Any], candidate_id: str) -> bool:
    return (
        scout.get("candidate_id") == candidate_id
        and scout.get("bucket") in BUCKETS
        and isinstance(scout.get("domains_confirmed"), list)
        and isinstance(scout.get("facets_collapsed"), list)
        and isinstance(scout.get("decomposition_depth_note"), str)
        and bool(scout["decomposition_depth_note"])
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for file in facts.get("files", []):
        ...
        if len(confirmed) >= 4:
            detections.append({
                ...
                "cluster_count": len(confirmed),
                "and_count": len(confirmed) - 1,
                "clusters": confirmed,
                "srp_sentence": "This file handles " + " and ".join(sorted(confirmed)) + ".",
                "evidence_scope": "function-name-cluster-syntax-only",
            })
    ...
    for candidate in candidates:
        ...
        try:
            ...
            continue
        if not _valid_scout(scout, candidate["candidate_id"]):
            missing_scouts.append(candidate["candidate_id"])
            continue
        bucket = scout["bucket"]
        recommendation = {
    ...
    if missing_scouts and status != "failed":
        status = "partial"
        failure = "c_scout_evidence_missing"
        code = 0
    payload = {
        "status": status,
    ...
    _atomic(
        output / "scan.json",
        json.dumps({"status": status, "failure_kind": failure, "c": facts}, indent=2, sort_keys=True) + "\n",
    )
    ...
    lines = [
        ...
        "",
        "C syntax nominates candidates only; the scout supplies the domain verdict.",
        "",
    ]
    lines.extend(
        f"- `{row['file']}` — `{row['bucket']}` (explicit scout); {row['recommendation']}"
        for row in findings
    )
    if missing_scouts:
