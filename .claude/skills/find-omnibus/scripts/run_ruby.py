#!/usr/bin/env python3
"""Carry Ruby syntax candidates through required human scout grading."""
...
GENERIC = frozenset({"load", "save", "validate", "authorize", "render", "write", "rotate", "find", "get", "set", "create", "update", "delete", "process", "handle"})
...
def main(argv: list[str] | None = None) -> int:
                ...
                detections.append(
                    {
                        ...
                        "clusters": confirmed,
                        "srp_sentence": "This file syntactically names " + " and ".join(sorted(confirmed)) + ".",
                    }
                )
    ...
    for candidate in candidates:
        ...
        if scout.get("candidate_id") != candidate["candidate_id"] or scout.get("bucket") not in BUCKETS:
            ...
            continue
        recommendation = {
            "confirmed_omnibus": "/refactor-subsystem <spec-id> (human authority required)",
    ...
    if missing_scouts and status == "complete":
        status = "partial"
        failure = "ruby_scout_evidence_missing"
        code = 2
    payload = {
        "status": status,
    ...
    _atomic(output / "scan.json", json.dumps({"status": status, "failure_kind": failure, "ruby": facts}, indent=2, sort_keys=True) + "\n")
