...
Use this guide for the bounded Kotlin/JVM family. Most consumers require an
exact schema-v1 `kotlin-project.json` and current
`.native-build/kotlin-build-evidence.json`. Complexity may instead retain
hash-bound authored `.kt` syntax leads when that fixture contract is absent;
it reports `partial` / `safe-defer-incomplete` and does not claim build-target
membership or native validation.
...
For clean-checkout dogfood, set `ARTIFACT_ROOT` to an absolute directory
...
- `find-complexity-hotspots`: `python3 -I -S "$SKILL_ROOT/scripts/run_kotlin.py" --project-root "$PWD" --kotlinc "$KOTLINC" --java "$JAVA" --target src --output-dir "$ARTIFACT_ROOT/complexity-hotspots/kotlin" --no-host-write`
