#!/usr/bin/env python3
"""Write the audit-decisions artifact contract from bounded Ruby comments."""
...
def _producer() -> ModuleType | None:
    ...
    return _module(path, "ruby_syntax_facts") if path.is_file() else None
...
def main(argv: list[str] | None = None) -> int:
    ...
    _atomic(output / "drift.md", rendered + f"\nRuby Prism syntax status: `{facts['status']}` via `{facts['analyzer']}`.\n")
