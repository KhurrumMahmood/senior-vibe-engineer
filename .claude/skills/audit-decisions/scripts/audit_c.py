#!/usr/bin/env python3
"""Write audit-decisions final artifacts from bounded C comment tokens."""
...
def _producer() -> ModuleType | None:
    ...
    return _module(path, "c_syntax_facts") if path.is_file() else None
...
def main(argv: list[str] | None = None) -> int:
    ...
    audit = _module(Path(__file__).with_name("audit.py"), "audit_decisions_c_base")
        ...
        for comment in file["comments"]:
            for match in REFERENCE.finditer(comment["text"]):
                references.append({
                    ...
                    "evidence_scope": "raw-comment-token-syntax-only",
                })
    ...
    _atomic(
        output / "drift.md",
        rendered
        + "\nC evidence is raw comment-token syntax only; it does not establish runtime applicability.\n",
    )
